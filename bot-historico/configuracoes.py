...
pacotes = ['keyring', 'selenium', 'pandas']   # Pacotes que serão utilizados pelo script. Caso necessário, inclua pacotes na lista, utilizando strings.
...
def adiciona_caminho(): # Aletrar o caminho para a pasta dos arquivos
    '''
    ...
    historico = input('Digite o caminho da pasta de histórico do arquivo: ').replace('\\','/')
    if not historico.endswith('/'):
        historico = historico + '/'
    ...
    keyring.set_password('MGOUV', 'historico', historico)   # Armazena o caminho da pasta de histórico de extrações, solicitado nas movimentações
